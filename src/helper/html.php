...
class html
{
     ...
     * @return void
     */
    public function multiSelect(string $name, array $options, array $tags = null, array $value = null)
    {
        $sValue    = 'value=""';
        $vSelected = (isset($value) && !empty($value))? $value: array();
        $sName  = (isset($name) && !empty($name))? 'name="'.$name.'"': null;
        $sField = (isset($name) && !empty($name))? 'id="field-'.$name.'"': null;
        if(isset($tags) && !empty($tags)){
            $sClass    = (isset($tags['class']) && !empty($tags['class']))? 'class="'.$tags['class'].'"': null;
        ...
        $select = "<select $sName $sField $sClass $sValue $sRequired $sSize $sMultiple $sDisabled multiple=\"multiple\">\n";
        ...
        foreach($options as $item){
            if(array_search($item['value'],$vSelected) !== false){
                $select .= '<option value="'.$item['value'].'" selected="selected" >'.$item['label']."</option>\n";
                continue;
            }
            $select .= '<option value="'.$item['value'].'" >'.$item['label']."</option>\n";
        }
